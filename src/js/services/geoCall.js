...
//in geograb call on index.js pass this checked radio button value as either 'ip' or 'manual'
    ...
    .then((jsonResponse) => {
      ...
      if (radioBtnVal === 'ip') {
        location.lat = jsonResponse['location']['lat'];
        location.lng = jsonResponse['location']['lng'];
      } else if (radioBtnVal === 'manual') {
        location.lat = jsonResponse.results[0].geometry.location.lat
        location.lng = jsonResponse.results[0].geometry.location.lng;
      }
      return location;
    })
    .catch((error) => {
